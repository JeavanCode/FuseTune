...
class Ensemble(nn.Module):
    def __init__(self, embed_dim, num_blocks, num_heads, mlp_ratio, num_classes):
        ...
        layer = 24
        self.model = beit_base_patch16_224_8k_vocab_cls_pt(img_size=args.image_size, init_values=0.1, use_shared_rel_pos_bias=True, num_classes=0,
                                                           use_abs_pos_emb=False)

        ckpt = torch.load(r'..\ckpt\beitv2_base_patch16_224_pt1k_300e.pth')['model']
        # ckpt['rel_pos_bias.relative_position_index'] = nn.Parameter(F.interpolate(ckpt['rel_pos_bias.relative_position_index'].to(torch.float32).unsqueeze(0).unsqueeze(0), size=(37, 37), mode='bicubic', align_corners=False).squeeze(0).squeeze(0))
        # ckpt['rel_pos_bias.relative_position_bias_table'] = nn.Parameter(F.interpolate(ckpt['rel_pos_bias.relative_position_bias_table'].to(torch.float32).unsqueeze(0).unsqueeze(0), size=(124, 12), mode='bicubic', align_corners=False).squeeze(0).squeeze(0))
...
parser.add_argument('--EXP_NUM', default='{Beit2-Cam-Fuse}', type=str)
...
if __name__ == '__main__':
    ...
    # model = torch.load(r'D:\Pycharm\Projects\Fuse\CheckPoints\{Beit2-Cam-Fuse}.pth')
    summary(model, input_size=(128, 3, args.image_size, args.image_size), depth=2, device='cpu')
