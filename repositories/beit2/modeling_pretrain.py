...
import sys
sys.path.append(r"D:\Pycharm\unilm\beit2")
...
class VisionTransformerForMaskedImageModelingCLS(VisionTransformerForMaskedImageModeling):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, vocab_size=8192, embed_dim=768, depth=12,
        ...
        # self.register_parameter('promote', nn.Parameter(torch.normal(mean=0, std=0.01, size=(12, 50, 768))))
    ...
    def forward_features(self, x, bool_masked_pos):
        ...
        rel_pos_bias = self.rel_pos_bias() if self.rel_pos_bias is not None else None
        for i, blk in enumerate(self.blocks):
            # x = torch.cat((x[:, :197, :], self.promote[i].expand(x.shape[0], -1, -1)), dim=1)
            x = blk(x, rel_pos_bias=rel_pos_bias)
    ...
    def forward(self, x, bool_masked_pos=None, return_all_tokens=True, return_patch_tokens=True):
        if bool_masked_pos is None:
            bool_masked_pos = torch.zeros((x.shape[0], self.patch_embed.num_patches), dtype=torch.bool).to(x.device)
