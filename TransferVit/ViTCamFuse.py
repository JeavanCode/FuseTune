...
from Utils.Utils import SmoothCrossEntropyLoss, get_params_groups, cosine_scheduler, interpolate_positional_encoding
...
class Ensemble(nn.Module):
    def __init__(self, embed_dim, num_heads, mlp_ratio, num_classes):
        ...
        self.model.positional_embedding.pos_embedding = interpolate_positional_encoding(self.model.positional_embedding.pos_embedding, new_num_patches=36)
        ...
        for _, (name, param) in enumerate(self.model.named_modules()):
            if fnmatch(name, 'transformer.blocks.*.pwff.fc2') or fnmatch(name, 'transformer.blocks.*.proj'):
...
parser.add_argument('--EXP_NUM', default='{ViT-Cam-Fuse}', type=str)
...
parser.add_argument("--root_path", default='D:/Pycharm/Projects/Fuse/EXP/', type=str)
...
if __name__ == '__main__':
    ...
    model = Ensemble(embed_dim=model_config['emb_dim'],
                     mlp_ratio=model_config['mlp_ratio'], num_heads=model_config['num_heads'],
                     num_classes=args.num_classes)
    summary(model, input_size=(128, 3, args.image_size, args.image_size), depth=2)
